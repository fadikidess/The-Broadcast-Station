...
if($usernames == ""){
    ...
    $target_dir_artposts = "TBS_uploads";
    $newtargdirart = "TBS_uploads/".date("h:i:s", time()).$usernames."post";
...
if($fileextension == "mp3"){
 if(move_uploaded_file($_FILES["file"]["tmp_name"], "TBS_uploads/".date("h:i:s", time()).$usernames."post.mp3")){
 ...
 echo "yes";
    ...
    rename(date("h:i:s", time()).$usernames.".txt", "TBS_uploads/".date("h:i:s", time()).$usernames.".txt"); 
    
    
    }   
    
}else{
    if(move_uploaded_file($_FILES["file"]["tmp_name"], "TBS_uploads/".date("h:i:s", time()).$usernames."post.".$fileextension)){
    ...
    rename(date("h:i:s", time()).$usernames."rate.txt", "TBS_uploads/".date("h:i:s", time()).$usernames."rate.txt"); 
    echo "yes";
...
<script>
  ...
  function post(){
     ...
     if(username !== null && username != ""){
          ...
          xhr.open("post","/postImage.php",true);
