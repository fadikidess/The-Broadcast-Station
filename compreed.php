...
<div class="search-container">
    ...
    <br>
    <input type="date" hidden id="now"/>
    <video width='100%' src="<?php echo $_REQUEST['id'];?>" id='vid'controls autoplay=""></video>
    <img alt="" src="<?php echo $_REQUEST['id'];?>" width='100%' ></img>
...
$reallines = file($file);
  ...
  for ($x = 0; $x < 4; $x++) {
      if($x == 0){
      $move = trim($reallines[$x]);
    echo "<h2>".$move."<div align='right'>Hosted by ".substr(substr($_REQUEST['id'], 0,-8),17)."</div></h2>";
    echo "<br><br>";    
    ...
    $move = trim($reallines[$x]);
    $start = trim($reallines[2]);
    echo " <button class='open-button' onclick='up()' id='submit'>Submit entry</button><div id='tex'></div><div id='dat'align='right'>Contest ends in ". $move  ."</div><script>
function up(){
   location.replace('/compup.php?rep=".$_REQUEST['id']."');        

}


let today = new Date().toISOString().substr(0, 10);
document.querySelector('#now').value = today;
var y = 0;
var m = 0;
var d = 0;
var fin = '".$move."';
document.querySelector('#now').valueAsDate = new Date();
        var init = document.getElementById('now').value.slice(0,2);

        y = parseFloat(fin.slice(0,4))-parseFloat(document.getElementById('now').value.slice(0,4));
        m = parseFloat(fin.slice(5,7))-parseFloat(document.getElementById('now').value.slice(5,7));
        d = parseFloat(fin.slice(8,10))-parseFloat(document.getElementById('now').value.slice(8,10));
  if(y > 0){
  var text = y + ' years '+m+' months and '+d+'  days left';
 document.getElementById('dat').innerHTML = text;
}else if(m >= 0 && y >= 0){  var text = y * 360 + 30 * m + d+' days left';
 document.getElementById('dat').innerHTML = text;}else if(d > 0 && m >= 0 && y >=0){  var text = y + ' years '+m+' months and '+d+'  days left';
 document.getElementById('dat').innerHTML = text;}else{
document.getElementById('dat').style.color = 'red';
document.getElementById('dat').innerHTML = 'The contest is over';
...
var start = '".$start."';
iy = parseFloat(start.slice(0,4))-parseFloat(document.getElementById('now').value.slice(0,4));
        im = parseFloat(start.slice(5,7))-parseFloat(document.getElementById('now').value.slice(5,7));
        id = parseFloat(start.slice(8,10))-parseFloat(document.getElementById('now').value.slice(8,10));
        var total = iy * 360 +im * 30 +id;
    document.getElementById('submit').style.display = 'none';  
 if(total > 0){
  var itext = 'Contest will start in '+total+' days';
document.getElementById('tex').innerHTML = itext;
...
<h1><?php $files = glob(substr($_REQUEST['id'],0,-8).'/*.{txt}', GLOB_BRACE);
$xs = 0;
...
$xs = $xs/2;
if($xs != 1){
echo 'This contest has '.$xs.' submissions';
}else{
    
echo 'This contest has one submission';
}
...
<?php
    ...
    $fileList = glob(substr($_REQUEST['id'],0,-8).'/*');
...
foreach($fileList as $filename){
   //Simply print them out onto the screen. 
    ...
    if($x < 5){
    if(substr($filename,-3,3) != "txt" && !is_dir($filename)){

        $myfile = fopen(substr($filename, 0,-8).".txt", "r+") or die("Unable to open file!");
        
   echo "<hr>","<a href='/compsubreed.php?id=".$filename."'<h3>". fgets($myfile)."<div align='center'>".substr($filename, 17,-27)."</div>";
  if(substr($filename,-3,3) != "mp4"){
   echo "<br><audio alt=''src='".$filename."' id='".$filename."'controls></audio><img src='".$filename."' width='250px'/><video src='".$filename."' width='250px'/>";
  }else{
       echo "<br><video src='".$filename."' id='".$filename."'width='250px'/>"."<div align='center'>".substr($filename, 17,-27)."</div>";
...
function logout(){
  ...
  function post(){
     ...
     if(username !== null && username != ""){
          ...
          xhr.open("post","/compreed.php",true);
