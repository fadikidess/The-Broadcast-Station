...
$initdate = $_POST["value4"];
$findate = $_POST["value5"];

    $target_dir_artposts = "TBS_comp";
    $newtargdirart = "TBS_comp/".date("h:i:s", time()).$usernames."post";
...
if($fileextension == "mp3"){
 if(move_uploaded_file($_FILES["file"]["tmp_name"], "TBS_comp/".date("h:i:s", time()).$usernames."post.mp3")){
 ...
 echo "yes";
    ...
    rename(date("h:i:s", time()).$usernames.".txt", "TBS_comp/".date("h:i:s", time()).$usernames.".txt"); 
    
    
    }   
    
}else{
    if(move_uploaded_file($_FILES["file"]["tmp_name"], "TBS_comp/".date("h:i:s", time()).$usernames."post.".$fileextension)){
    
     

        mkdir("TBS_comp/".date("h:i:s", time()).$usernames);
    
    $myfile = fopen(date("h:i:s", time()).$usernames.".txt", "w");
    rename(date("h:i:s", time()).$usernames.".txt", "TBS_comp/".date("h:i:s", time()).$usernames.".txt"); 
    fwrite($myfile,$description."\n".$descriptions."\n".$initdate."\n".$findate."\n");
        $myfiles = fopen(date("h:i:s", time()).$usernames."win.txt", "w");
    rename(date("h:i:s", time()).$usernames."win.txt", "TBS_comp/".date("h:i:s", time()).$usernames."win.txt"); 
    echo "yes";
...
<div class="search-container">
    ...
    <form target="_top" action="" method='post' id="form_up"class="form-container"  name="form" enctype="multipart/form-data">
        ...
        <input type="file" name="file" class="file"id="files"accept="file_extension|audio/*|video/*|image/*"onchange="changefile()"/>
    ...
    <input type="text" width='50px' height="30px"placeholder="Description..."style="background-color: #f2f2f2;width: 390px;"id="desc"/>
        <p id="er"></p>
        <input type="date" id="initdate"/><input type="date" id="findate"/>
        <br>
...
<script>
  ...
  function post(){
     ...
     if(username !== null && username != ""){
          if(document.getElementById("title").value != "" && document.getElementById("desc").value!= "" && document.getElementById("initdate").value != ""&& document.getElementById("findate").value != ""){
          ...
          data.append("value4",document.getElementById("initdate").value);
          data.append("value5",document.getElementById("findate").value);
          data.append("file",document.getElementById("files").files[0]);
          xhr.open("post","/createcomp.php",true);
